...
BookOasis 대시보드/카테고리 탭에 픽시브(Pixiv) 랭킹을 카드 형태로 보여주는 메타데이터 플러그인입니다.
검색/적용(search/apply) 기능은 없고, **대시보드 위젯 + 카테고리 풀페이지 탭 전용**입니다.
...
- 공통 데스크 카드 그리드 위젯(`dashboard_widget`)과 좌측 카테고리 메뉴의 풀페이지 탭
  (`category_tab`) 두 곳에 동시 노출
...
5. 좌측 메뉴 또는 공통 데스크 화면에서 카드가 보이는지 확인합니다.
