...
class PixivRankingMetadataProvider(BaseMetadataProvider):
    ...
    config_schema = [
        ...
        },
        {
            ...
            "default": "240x480",
            "options": [
                {"value": "128x128", "label": "작게 (128x128, 가장 빠름)"},
                {"value": "240x480", "label": "보통 (240x480, 권장)"},
                {"value": "360x360_70", "label": "크게 (360x360)"},
                {"value": "480x960", "label": "원본 그대로 (480x960, 가장 느림)"},
            ],
        },
    ]
    ...
    def _shrink_thumb_url(self, thumb_url, target="240x480"):
        """Pixiv 썸네일 URL의 사이즈 세그먼트(/c/480x960/...)를 설정된 크기로 교체.
        대시보드 카드는 어차피 작게 표시되므로 큰 원본 썸네일을 받을 필요가 없음."""
        if not thumb_url:
            return thumb_url
        return re.sub(r"/c/\d+x\d+(_\d+)?/", f"/c/{target}/", thumb_url)
    ...
    def _fetch_thumb_data_uri(self, thumb_url, session_id, thumb_size):
        """썸네일을 다운로드해 base64 data URI로 변환 (로컬 저장 없음)."""
        thumb_url = self._shrink_thumb_url(thumb_url, thumb_size)
        if not thumb_url:
    ...
    def _build_items(self, contents, session_id, thumb_size):
        logger.warning(
        ...
        def _work(idx, entry):
            ...
            image_data_uri = self._fetch_thumb_data_uri(thumb_url, session_id, thumb_size)
    ...
    def get_dashboard_data(self, db_type, limit=10):
        ...
        thumb_size = cfg.get("THUMB_SIZE", "240x480")
