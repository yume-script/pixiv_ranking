...
[성능 개선 내역]
- 랭킹 API 응답에 대한 TTL 캐시 추가 (기본 5분)
- 썸네일 base64 데이터 URI 자체를 URL 기준으로 캐시 (기본 30분)
- requests.Session()으로 커넥션(TCP/TLS) 재사용
- 상세 로그를 warning -> debug로 낮춤 (에러성 로그만 warning 유지)
...
_THUMB_SIZE_RE = re.compile(r"/c/\d+x\d+(?:[^/]*)?/")
